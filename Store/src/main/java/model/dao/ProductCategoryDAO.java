...
public class ProductCategoryDAO {
	...
	public  ArrayList<ProductCategory> getProducCategoryList() {
		...
		try {
			...
			while (rs.next()) {
				ProductCategory Product = new ProductCategory(rs.getString("product.id_product"), rs.getString("product.name"), rs.getInt("product.size"),rs.getInt("product.price"),rs.getString("category.name"),rs.getString("product.image"));
				result.add(Product);
			}
	...
	public ProductCategory  productCate(String id) {
		...
		ArrayList<ProductCategory> list =dao.getProducCategoryList();
